...
describe("Parser", () => {
  ...
  describe("LambdaExpression", () => {
    ...
    it("returns a lambda expression ast on parse success", () => {
      const node = Parser.language.lambdaExpression.tryParse("number -> number")
      const parameters = [AST.createIdentifier("number")]
      const result = AST.createIdentifier("number")
      const lambdaExpression = AST.createLambdaExpression(parameters, result)
